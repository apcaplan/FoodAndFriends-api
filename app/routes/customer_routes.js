...
// pull in Mongoose model for customers
...
router.get('/customers', requireToken, (req, res, next) => {
  Customer.find( ({ owner: req.user.id }))
    .then(customers => {
      // `customers` will be an array of Mongoose documents
      // we want to convert each one to a POJO, so we use `.map` to
      // apply `.toObject` to each one
      return customers.map(customer => customer.toObject())
    })
    // respond with status 200 and JSON of the customers
...
// SHOW
// GET /customers/5a7db6c74d55bc51bdf39793
router.get('/customers/:id', requireToken, (req, res, next) => {
  // req.params.id will be set based on the `:id` in the route
  Customer.findById(req.params.id)
    .then(handle404)
    // if `findById` is succesful, respond with 200 and "customer" JSON
    .then(customer => res.status(200).json({ customer: customer.toObject() }))
    // if an error occurs, pass it to the handler
    .catch(next)
})
...
// UPDATE
// PATCH /customers/5a7db6c74d55bc51bdf39793
router.patch('/customers/:id', requireToken, removeBlanks, (req, res, next) => {
...
// DESTROY
// DELETE /customers/5a7db6c74d55bc51bdf39793
